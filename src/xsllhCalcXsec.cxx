...
int main(int argc, char** argv)
{
    ...
    bool throw_eff = false;
    bool calc_true_xsec=false;
    std::string json_file;
    std::string input_file;
    std::string output_file;
    unsigned int num_toys{0};
 
    bool fit_hydrogen = false;
    int idxH = -1;
    int fluxrw = -1;

    char option;
    while((option = getopt(argc, argv, "j:i:o:n:f:r:pmthex")) != -1)
    {
        switch(option)
        {
            case 'j':
            ...
            case 'f':
                fit_hydrogen = true;
                idxH = std::stoi(optarg);
                break;
            case 'r':
                fluxrw = std::stoi(optarg);
                break;
            case 'x':
                calc_true_xsec = true;
                break;
    ...
    if(num_toys != 0)
        xsec.GenerateToys(num_toys,throw_eff,fluxrw);
    else
        xsec.GenerateToys(throw_eff,fluxrw);
    xsec.CalcCovariance(use_best_fit, fit_hydrogen, idxH);
