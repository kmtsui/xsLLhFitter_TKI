...
FitParameters::FitParameters(const std::string& par_name, bool random_priors, bool tkiFit, bool fixParameters, bool tkiConstraint)
{
    m_name = par_name;
    m_rng_priors = random_priors;
    m_TKI_fit = tkiFit;
    m_TKI_constraint = tkiConstraint;
    m_fixParameters = fixParameters;
...
void FitParameters::InitParameters()
{
    ...
    for(const auto& sig : v_signals)
    {
        ...
        for(int i = 0; i < nbins; ++i)
        {
            ...
            pars_fixed.push_back(m_fixParameters);
            //if (i!=nbins-2) pars_fixed.push_back(false);
            //else pars_fixed.push_back(true);
        }
                                         ...
                                         RegMethod flag) const
{
    ...
    */
    ...
    for(const auto& signal_id : v_signals)
    {
        ...
        if(flag == kL1Reg)
        {
            for(int i = offset; i < offset+nbins-1-1; ++i)  //exclude OOPS bin //exclude H bin
...
double FitParameters::GetChi2(const std::vector<double>& params) const

{
    if(params.size() != pars_prior.size())
    {
        std::cout << WAR << "In FitParameters::GetChi2()\n"
                  << WAR << "Dimension check failed. Returning zero." << std::endl;
        return 0.0;
    }

    if (!m_TKI_fit) return 0.;
    const unsigned int nbins = m_signal_bins.at(0).size();
    double chi2 = 0;
    for(int i = nbins-2; i < nbins-1; ++i)                //only the H bin
           chi2 += (params[i]-1.)*(params[i]-1.)/(0.3*0.3);
    if (!m_TKI_constraint) return chi2;
    if (nbins==6) chi2 += (params[3]-1.)*(params[3]-1.)/(0.25);        //special treatment for pN
    if (nbins==7) chi2 += (params[0]-1.)*(params[0]-1.)/(0.25);        //special treatment for dptt
    if (nbins==7) chi2 += (params[4]-1.)*(params[4]-1.)/(0.25);        //special treatment for dptt
    if (nbins==10) {
        chi2 += (params[5]-1.)*(params[5]-1.)/(0.3*0.3)-(params[8]-1.)*(params[8]-1.)/(0.3*0.3);
        chi2 += (params[0]-1.)*(params[0]-1.)/(0.25)+(params[4]-1.)*(params[4]-1.)/(0.25);  
    }
    return chi2;
}
