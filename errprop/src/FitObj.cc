...
FitObj::FitObj(const std::string& json_config, const std::string& event_tree_name,
               bool is_true_tree, bool use_det_binning)
{
    OptParser parser;
    ...
    AnaTreeMC event_tree(fname_mc.c_str(), event_tree_name, true);
...
void FitObj::GetFluxParPos(int *fluxparpos)
{
    fluxparpos[0] = fit_par[0]->GetNpar();
    fluxparpos[1] = nfluxpar;
                                ...
                                const std::string& det)
{
    ...
    else
    {
        ...
        for(int i = 1; i <= flux_hist.GetNbinsX(); ++i)
        {
            ...
            flux_int += val * flux_hist.GetBinWidth(i)*flux_par[idx];
            //std::cout<<"enu = "<<enu<<" val = "<<val<<" idx = "<<idx<<" flux_par[idx] = "<<flux_par[idx]<<std::endl;
        }
    }
...
TMatrixDSym FitObj::GetPrefitCov()
{
    ...
    for(const auto& param_type : fit_par)
    {
        ...
        std::cout << TAG << param_type->GetName()<<" cov_mat->GetNrows() = " << cov_mat->GetNrows()
                  << std::endl;
