...
XsecFitter::XsecFitter(TDirectory* dirout, const int seed, const int num_threads, bool minosErr, bool throwPrefit)
...
void XsecFitter::InitFitter(std::vector<AnaFitParameters*>& fitpara)
{
    ...
    for(std::size_t i = 0; i < m_fitpara.size(); i++)
    {
        ...
        if (m_throw_prefit) m_fitpara[i]->ThrowPrefit(vec1);
    ...
    for(int i = 0; i < m_fitpara.size(); ++i)
    {
        ...
        {
            h_prefit.SetBinContent(num_par, m_fitpara[i]->GetParOriginal(j));//h_prefit.SetBinContent(num_par, m_fitpara[i]->GetParPrior(j));
...
bool XsecFitter::Fit(const std::vector<AnaSample*>& samples, int fit_type, bool stat_fluc)
{
    ...
    {
      	std::vector<std::vector<double>> fitpar_nominal;
    	for(const auto& fitpar : m_fitpara)
      	{
        	std::vector<double> toy_nominal(fitpar->GetNpar(), 0.0);
        	fitpar -> GetParOriginal(toy_nominal);
        	fitpar_nominal.emplace_back(toy_nominal);
      	}
...
#pragma omp parallel for num_threads(m_threads)
        	for(unsigned int i = 0; i < N; ++i)
        	{
           		AnaEvent* ev = m_samples[s]->GetEvent(i);
            		...
            		for(int j = 0; j < m_fitpara.size(); ++j)
                		m_fitpara[j]->ReWeight(ev, det, s, i, fitpar_nominal[j]);
        	}
    ...
    //MINOS error
    if (m_minos) for (int i=0;i<m_fitter->NDim();i++) {
      double par_err_up, par_err_low;
      if (!m_fitter->GetMinosError(i, par_err_low, par_err_up))
        std::cout << TAG << "MINOS error failed for parameter "<<i<<std::endl
                  << TAG << "Status code: " << m_fitter->Status() << std::endl;
    }
...
#pragma omp parallel for num_threads(m_threads)
        ...
        chi2 += sample_chi2;
        //if (s!=0) chi2 += sample_chi2;
        //else if (s==0) chi2 +=m_samples[s]->Calc2DChi2();
        //if (s==0) chi2 +=m_samples[s]->GetPionMomChi2();
...
void XsecFitter::SaveEvents(int fititer)
{
    ...
    if (fititer==0) // save prefit evhist_all
    {
        const int evhistN = m_samples.size();
        int totalBins = 0;
        TH1D* evhist_prefit[evhistN];
        for(size_t s = 0; s < m_samples.size(); s++)
        {
          evhist_prefit[s]=m_samples[s]->GetPredHisto();
          totalBins+=evhist_prefit[s]->GetNbinsX();
        }
        TH1D* evhist_prefit_all = new TH1D("evhist_prefit_all","evhist_prefit_all",totalBins,0,totalBins);
        unsigned int bin = 1;
        for(size_t s = 0; s < m_samples.size(); s++)
        {
          for(int i = 1; i <= evhist_prefit[s]->GetNbinsX(); ++i)
            evhist_prefit_all->SetBinContent(bin++, evhist_prefit[s]->GetBinContent(i));
        }
        evhist_prefit_all->Write();
    }
}
...
void XsecFitter::SaveFinalEvents(int fititer, std::vector<std::vector<double>>& res_params)
{
    ...
    const int evhistN = m_samples.size();
    int totalBins = 0;
    TH1D* evhist_postfit[evhistN];
    TH1D* evhist_data[evhistN];
    for(size_t s = 0; s < m_samples.size(); s++)
    {
        evhist_postfit[s]=m_samples[s]->GetPredHisto();
        evhist_data[s]=m_samples[s]->GetDataHisto();
        totalBins+=evhist_postfit[s]->GetNbinsX();
    ...
    TH1D* evhist_postfit_all = new TH1D("evhist_postfit_all","evhist_postfit_all",totalBins,0,totalBins);
    TH1D* evhist_data_all = new TH1D("evhist_data_all","evhist_data_all",totalBins,0,totalBins);
    unsigned int bin = 1;
    for(size_t s = 0; s < m_samples.size(); s++)
    {
        for(int i = 1; i <= evhist_postfit[s]->GetNbinsX(); ++i){
            evhist_postfit_all->SetBinContent(bin, evhist_postfit[s]->GetBinContent(i));
            evhist_data_all->SetBinContent(bin, evhist_data[s]->GetBinContent(i));
            bin++;
        }
    }
    evhist_postfit_all->Write();
    evhist_data_all->Write();
    m_dir->cd();
    outtree->Write();
}
                             ...
                             const std::vector<std::vector<double>>& par_errors)
{
    for(std::size_t i = 0; i < m_fitpara.size(); i++)
    {
        ...
        if (m_throw_prefit) {
            m_fitpara[i]->GetParPrefit(par_original);
        }
