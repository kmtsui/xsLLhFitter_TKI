...
int treeConvert_pN(TString inFileName="/hepstore/kmtsui/T2K/work/highland2/myNumuCCMultiPiAnalysis/v0r0/output/run*.root", TString inTreeName="default",  TString inTreeName_T="truth", TString outFileName="../inputs/neut_Run8_pN.root",
                Float_t sigWeight=1., Int_t EvtStart=0, Int_t EvtEnd=0,
                TString D1NameRec="recDeltapTT_3TPCAllSec", TString D1NameTrue="truthDeltapTT_vtx", TString D1NameTrue_T="truthpN",
                TString D2NameRec="selmu_mom", TString D2NameTrue="selmu_truemom", TString D2NameTrue_T="truelepton_mom")
{
  ...
  //outFileName="../inputs/neut_prod6D_dataPOT_pN_H.root";

  //inFileName="/scratch/kmtsui/runOutput_run2_4/run*.root";
  //sigWeight=11.53/(62.7+109.7);
  //outFileName="../inputs/neut_Run2_8_pN_dataPOT_H.root";
  //outFileName="../inputs/neut_Run2_8_pN_test_mult_2.root";

  /*TFile *fHist2d = new TFile("/scratch/kmtsui/runOutput_run2_4/hist2d.root");
  TH2D* hist2d = (TH2D*)fHist2d->Get("truthpN_vs_truepi_mom");
...
//std::cout<<"jentry = "<<jentry<<std::endl;
    ...
    //for(int i=0; i<nbranches; i++){
      bool passed = 0;
      bool commonVtxCut = (abs(selmu_pos[0]-TPCProtonPosStart[0][0])<50&&abs(selmu_pos[0]-PosPionPosStart[0][0])<50&&abs(TPCProtonPosStart[0][0]-PosPionPosStart[0][0])<50&&abs(selmu_pos[1]-TPCProtonPosStart[0][1])<50&&abs(selmu_pos[1]-PosPionPosStart[0][1])<50&&abs(TPCProtonPosStart[0][1]-PosPionPosStart[0][1])<50&&abs(selmu_pos[2]-TPCProtonPosStart[0][2])<30&&abs(selmu_pos[2]-PosPionPosStart[0][2])<30&&abs(TPCProtonPosStart[0][2]-PosPionPosStart[0][2])<30);
...
//std::cout<<"accum_level"<<std::endl;
      ...
      weight = weight*sigWeight; //scale to data POT

      /*double weightMult = 1.;
      int pbin = (truepi_mom-150)/52.5;
      int thetabin = (truthpN)/75.;
    ...
    //if (!kpsCut) D1Reco=1550; //OOPS bin
...
//std::cout<<"OOPS"<<std::endl;
    ...
    outtree->Fill();
      ...
      /*if(accum_level[0][i]>accumToCut[i]){
        ...
        bool commonVtxCut = (abs(selmu_pos[0]-TPCAllSecPosStart[0][0])<50&&abs(selmu_pos[0]-TPCAllSecPosStart[1][0])<50&&abs(TPCAllSecPosStart[1][0]-TPCAllSecPosStart[0][0])<50&&abs(selmu_pos[1]-TPCAllSecPosStart[0][1])<50&&abs(selmu_pos[1]-TPCAllSecPosStart[1][1])<50&&abs(TPCAllSecPosStart[1][1]-TPCAllSecPosStart[0][1])<50&&abs(selmu_pos[2]-TPCAllSecPosStart[0][2])<30&&abs(selmu_pos[2]-TPCAllSecPosStart[1][2])<30&&abs(TPCAllSecPosStart[1][2]-TPCAllSecPosStart[0][2])<30);
