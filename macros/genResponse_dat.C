...
{ 
  ...
  string outname = "_dat_allsample.root";
  if (DISMultiPi>-1) outname = Form("_%d_dat_allsample.root",DISMultiPi);
  ...
  const double Rbins[nRbins+1] = {0,60,120,180,190};
  //const double Rbins[nRbins+1] = {0,30,60,90,120,150,180,190};
 ...
 // const double Tbins[nTbins+1] = { -300, -279, -259, -238, -217, -197, -176, -155, -134, -114, -93, -72, -52, -31, -10, 10, 31, 52, 72, 93 ,114, 134, 155, 176, 197, 217, 238, 259, 279, 300};
  const double Tbins[nTbins+1] = {0,60,120,180};
  //const double Tbins[nTbins+1] = {0,30,60,90,120,150,180};
 ...
 for (int sample =0;sample<6;sample++) {
  ...
  Double_t pN[nTopologies], pNTrue[nTopologies], dat[nTopologies], datTrue[nTopologies];
  ...
  for (Long64_t ev=0;ev<nentries;ev++) {
    ...
    int rBin = -1;
    int tBin = -1;
    if (sample==5) dat[SelectedSample]=0.1;
    for (int i=0;i<nRbins;i++) {
      if (dat[SelectedSample]>Rbins[i] && dat[SelectedSample]<Rbins[i+1]) {rBin=i;break;}
    }
    for (int i=0;i<nTbins;i++) {
      if (datTrue[SelectedSample]>Tbins[i] && datTrue[SelectedSample]<Tbins[i+1]) {tBin=i;break;}
    }
    ...
    for(int w = 0; w < dialGraph->GetN(); w++){
      ...
      if (tBin>-1) { 
        if (topo_target[SelectedSample]!=1)  hreco[w][tBin][evtTopo][0]->Fill(dat[SelectedSample],y[w]*FluxWeight[SelectedSample]);
        else hreco[w][tBin][evtTopo][1]->Fill(dat[SelectedSample],y[w]*FluxWeight[SelectedSample]);
      }
    }
    /*if (dialGraph->GetN()>1) {
  ...
  /*for(const int w = 0; w < nWeights; w++){
    for(const int bt = 0; bt < nTbins; bt++){
      ...
      //for(const int t = 0; t < nTopologies; t++){
        ...
        inpRwFly->AddReweighting(rwPar);false
