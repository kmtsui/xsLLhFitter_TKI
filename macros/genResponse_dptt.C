...
{ 
  ...
  const double Rbins[nRbins+1] = {-700,-300,-100,100,300,700,800};
 ...
 // const double Tbins[nTbins+1] = { -300, -279, -259, -238, -217, -197, -176, -155, -134, -114, -93, -72, -52, -31, -10, 10, 31, 52, 72, 93 ,114, 134, 155, 176, 197, 217, 238, 259, 279, 300};
  const double Tbins[nTbins+1] = {-700,-300,-100,100,300,700};
 ...
 for (int sample =0;sample<6;sample++) {
  ...
  for (Long64_t ev=0;ev<nentries;ev++) {
    ...
    int tBin = -1;
    int pBin = -1;
    if (sample==5) dptt[SelectedSample]=0.1;
    if (sample!=0) Pp[SelectedSample]=500.;
    if (sample==0&&dptt[SelectedSample]>700) Pp[SelectedSample]=500.;
    for (int i=0;i<nRbins;i++) {
      if (dptt[SelectedSample]>=Rbins[i] && dptt[SelectedSample]<Rbins[i+1]) {rBin=i;break;}
    }
    for (int i=0;i<nTbins;i++) {
      if (dpttTrue[SelectedSample]>=Tbins[i] && dpttTrue[SelectedSample]<Tbins[i+1]) {tBin=i;break;}
    }
