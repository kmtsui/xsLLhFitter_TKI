...
void genResponse(string microtreeIn="/hepstore/kmtsui/T2K/work/GlobalAnalysisTools/T2KReWeight/output/run*.root", string rwpath="", string dialName="CA5", double nom=1., double err=0.12/1.01, int DISMultiPi = -1) 
{ 
  ...
  const double Rbins[nRbins+1] = {1080,1152,1224,1296,1368,1440,1512,1584,1656,1728,1800,1872};
 ...
 // const double Tbins[nTbins+1] = { -300, -279, -259, -238, -217, -197, -176, -155, -134, -114, -93, -72, -52, -31, -10, 10, 31, 52, 72, 93 ,114, 134, 155, 176, 197, 217, 238, 259, 279, 300};
  const double Tbins[nTbins+1] = {1080,1152,1224,1296,1368,1440,1512,1584,1656,1728,1800};
 ...
 for (int sample =0;sample<6;sample++) {
  ...
  Double_t W[nTopologies], WTrue[nTopologies], costhA[nTopologies], costhATrue[nTopologies], phiA[nTopologies], phiATrue[nTopologies];
    Int_t topo_np[nTopologies], topo_target[nTopologies], topo_npi[nTopologies];
  ...
  if (sample<5) {
    ...
    sumT->SetBranchAddress("W", &recVar);
  }
  sumT->SetBranchAddress("dpttTrue", &dpttTrue);
  sumT->SetBranchAddress("WTrue", &truthVar);
  ...
  for (Long64_t ev=0;ev<nentries;ev++) {
    ...
    for(int w = 0; w < dialGraph->GetN(); w++){
      ...
      if (tBin>-1) { 
        if (topo_target[SelectedSample]!=1)  hreco[w][tBin][evtTopo][0]->Fill(dptt[SelectedSample],y[w]*FluxWeight[SelectedSample]);
        else hreco[w][tBin][evtTopo][1]->Fill(dptt[SelectedSample],y[w]*FluxWeight[SelectedSample]);
      }
    }
    /*if (dialGraph->GetN()>1) {
