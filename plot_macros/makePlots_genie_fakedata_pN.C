...
void makePlots(){
  ...
  TFile* fIn = new TFile("xsec_neutMC_genieData_pN.root");
  ...
  TFile* fIn_neut = new TFile("xsec_neut_pN.root");
  TH1D* sel_best_fit_neut = (TH1D*)fIn_neut -> Get("sel_best_fit");
  TFile* fIn_genie = new TFile("xsec_genie_pN.root");
...
void makePlots_genie_fakedata_pN(){
  makePlots();
}
