...
void makePlots_TKI(int TKI){
    ...
    switch (TKI) {
        case 0:
            mcfile = "neut_prod6T_dptt_geomfixed.root";
            datafile = "neut_prod6T_dptt_geomfixed_SIpion.root";
            nBins = nDpttbins;
            for (int i=0;i<nDpttbins+1;i++) Bins[i]=Dpttbins[i];
            tkimax = 700;
            xlabel = "Reconstructed #it{#deltap_{TT}} (MeV/c)";
            ...
            break;
        case 1:
            mcfile = "neut_prod6T_pN_geomfixed.root";
            datafile = "neut_prod6T_pN_geomfixed_SIpion.root";
            nBins = nPnbins;
            ...
            break;
        case 2:
            mcfile = "neut_prod6T_dat_geomfixed.root";
            datafile = "neut_prod6T_dat_geomfixed_SIpion.root";
            nBins = nDatbins;
    ...
    TH1D* hSIpion_MC = new TH1D("","",100,0,2);
    TH1D* hSIpion_Data = new TH1D("","",100,0,2);
    TH1D* hWeight_MC = new TH1D("","",100,0,0.25);
    TH1D* hWeight_Data = new TH1D("","",100,0,0.25);

    TH1D* hSelectedEvents_MC =new TH1D("","",nBins,Bins);
    TH1D* hSelectedEvents_Data =new TH1D("","",nBins,Bins);
    ...
    selectedEvents->SetBranchAddress("D2Rec", &D2Reco);
    selectedEvents->SetBranchAddress("weight", &weight);
    selectedEvents->SetBranchAddress("weight", &weightNom);
    selectedEvents->SetBranchAddress("weight", &weightMC);
    selectedEvents->SetBranchAddress("weight_pionSI", &weight_SIpion);
    ...
    for (int i=0;i<selectedEvents->GetEntries();i++) { 
        ...
        hSIpion_MC->Fill(weight_SIpion,weightMC);
        hWeight_MC->Fill(weightMC);
        //hSIpion_MC->Fill(weightMC);
        if (topology==2||topology==3)
            hSelectedEvents_MC->Fill(D1true,weightMC);
    }
    THStack *hs_evhist_prefit[nSamples];
    ...
    selectedEventsData->SetBranchAddress("D2Rec", &D2Reco);
    selectedEventsData->SetBranchAddress("weight", &weight);        
    selectedEventsData->SetBranchAddress("weight_pionSI", &weight_SIpion); 
    selectedEventsData->SetBranchAddress("D1True", &D1true);
    selectedEventsData->SetBranchAddress("topology", &topology);
    for (int i=0;i<selectedEventsData->GetEntries();i++) { 
        selectedEventsData->GetEntry(i);
        if (weight>10) continue;
        hSIpion_Data->Fill(weight_SIpion,weight);
        hWeight_Data->Fill(weight);
        //hSIpion_Data->Fill(weight);
        ...
        if (topology==2||topology==3) 
            hSelectedEvents_Data->Fill(D1true,weight);
    }
    for (int i=0;i<nSamples;i++) {
    ...
    TCanvas* cSIpion = new TCanvas();
    hSIpion_MC->SetLineColor(kBlack);
    hSIpion_Data->SetLineColor(kRed);
    hSIpion_MC->GetXaxis()->SetTitle("weight_SIpion");
    hSIpion_MC->GetYaxis()->SetTitle("Count");
    hSIpion_MC->Draw("hist");
    hSIpion_Data->Draw("hist same");
    auto legend_SIpion = new TLegend(0.6,0.5,0.9,0.8);
    legend_SIpion->AddEntry(hSIpion_MC,"NEUT 6T","l");
    legend_SIpion->AddEntry(hSIpion_Data,"NEUT 6T (reweighted)","l");
    legend_SIpion->Draw();
    cSIpion->SaveAs("SIpion.pdf");

    hWeight_MC->SetLineColor(kBlack);
    hWeight_Data->SetLineColor(kRed);
    hWeight_MC->GetXaxis()->SetTitle("Event weight");
    hWeight_MC->GetYaxis()->SetTitle("Count");
    hWeight_MC->Draw("hist");
    hWeight_Data->Draw("hist same");
    legend_SIpion->Draw();
    cSIpion->SaveAs("weight.pdf");

    hSelectedEvents_MC->SetLineColor(kBlack);
    hSelectedEvents_Data->SetLineColor(kRed);
    hSelectedEvents_MC->GetXaxis()->SetTitle(xlabel.c_str());
    hSelectedEvents_MC->GetYaxis()->SetTitle("Count");
    hSelectedEvents_MC->Draw("hist");
    hSelectedEvents_Data->Draw("hist same");
    legend_SIpion->Draw();
    cSIpion->SaveAs(Form("%s_selectedEvents.pdf",varName.c_str()));

    TH1D* hEfficiency_ratio = (TH1D*)hSelectedEvents_MC->Clone();
    hEfficiency_ratio->Add(hSelectedEvents_Data,-1);
    hEfficiency_ratio->Divide(hSelectedEvents_MC);
    hEfficiency_ratio->GetYaxis()->SetTitle("#delta#epsilon/#epsilon");
    hEfficiency_ratio->Draw("hist");
    cSIpion->SaveAs(Form("%s_efficiency.pdf",varName.c_str()));
    ...
    for (int j=0;j<nSamples;j++) {
        ...
        TH1D* htmp = (TH1D*)getTH1(hs_evhist_prefit[j]);//(TH1D*)hs_evhist_prefit[j]->GetHistogram();//(TH1D*)hs_evhist_prefit[j]->Clone();
        TH1D* hRatio = (TH1D*)hData[j]->Clone();
        //hRatio->Add(hData[j],-1);
        for (int k=1;k<=hRatio->GetNbinsX();k++) {
            hRatio->SetBinContent(k,(htmp->GetBinContent(k)-hRatio->GetBinContent(k))/hData[j]->GetBinError(k));
            hRatio->SetBinError(k,0);
        }
        hRatio->GetXaxis()->SetTitle(xlabel.c_str());
        hRatio->GetYaxis()->SetTitle("diff/MC stat. error");
        hRatio->GetYaxis()->SetRangeUser(0,1);
        hRatio->Draw("hist");
        if (j==0) c1->SaveAs(Form("%s_ratio_sig.pdf",varName.c_str()));
        else c1->SaveAs(Form("%s_ratio_cr%i.pdf",varName.c_str(),j));
        if (j>0&&TKI==2){
