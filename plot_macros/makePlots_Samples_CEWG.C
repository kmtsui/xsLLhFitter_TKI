...
void makePlots_TKI(int TKI){
    ...
    TH1D* hData_pmom = new TH1D("","",nPmombins,Pmombins);
    TH1D* hMCTruth = new TH1D("","",nBins,Bins);
    ...
    std::string topo_multipi_types_reduced[]={"CC1#pi^{+}Xp","CC-other","NC, #bar{#nu}_{#mu}, #nu_{e}, #bar{#nu}_{e}","OOFV","OOPS"};
    //                                              0           1        2                                             3        4
    int nTopo_reduced = 5;
    int topology_colors_reduced[]={4,2,3,6,1};
    int topo_map[]={1,1,0,0,1,1,1,1,1,2,3,4};
    ...
    for (int i=0;i<selectedEvents->GetEntries();i++) { 
        selectedEvents->GetEntry(i);
        if(topology<0&&topology>11) continue;
        //if (topology==2||topology==3) if (D1true>-300&&D1true<-100) weightMC*=1.5;
        //if (topology!=2&&topology!=3) weightMC=weight*1.5;
        hMC_Prefit[sample][topo_map[topology]]->Fill(D1Reco,weight);
        hMC_2D[sample][0]->Fill(D1Reco,D2Reco,weightMC);
        hMC_2D[sample][1]->Fill(D1Reco,D2Reco,weight);
        if (topology==2||topology==3) hMCTruth->Fill(D1true,weight);
        if (abs(D1Reco)<tkimax) {
    ...
    int topology_colors[nTopo]= {2        , 3              , 4              , 7              , 0, 31,  51                   , 5                 ,48           ,6 , 1,8};
    for (int i=0;i<nTopo_reduced;i++) {
        for (int j=0;j<nSamples;j++){
            hMC_Prefit[j][nTopo_reduced-1-i]->SetLineColor(topology_colors_reduced[nTopo_reduced-1-i]);
            hMC_Prefit[j][nTopo_reduced-1-i]->SetFillColor(topology_colors_reduced[nTopo_reduced-1-i]);
            hMC_Prefit[j][nTopo_reduced-1-i]->SetFillStyle(3154);
            hs_evhist_prefit[j]->Add(hMC_Prefit[j][nTopo_reduced-1-i]);
        }
        hMC_Prefit_pmom[nTopo_reduced-1-i]->SetLineColor(topology_colors_reduced[nTopo_reduced-1-i]);
        hMC_Prefit_pmom[nTopo_reduced-1-i]->SetFillColor(topology_colors_reduced[nTopo_reduced-1-i]);
        hMC_Prefit_pmom[nTopo_reduced-1-i]->SetFillStyle(3154);
    ...
    for (int j=0;j<nSamples;j++) {
        ...
        TLegend* legendTopo;
        //if (!(TKI==1&&j==0)&&!(TKI==2&&j>0)) continue;
        if (TKI!=2) legendTopo = new TLegend(0.62,0.45,0.92,0.9);
        //else if (TKI==2) legendTopo = new TLegend(0.18,0.39,0.58,0.89);
        else legendTopo = new TLegend(0.,0.,1,1);
        //if (j>0) legendTopo->AddEntry((TObject*)0, samName[j].c_str(), "");
        ...
        for (int i=0;i<nTopo_reduced;i++){
            ...
            //double frac = nMCEvents[j][i]/nMCEventsSum[j]*100.;
            //std::cout<<"frac = "<<nMCEvents[j][i]<<"/"<<nMCEventsSum[j]<<"="<<frac<<std::endl;
            legendTopo->AddEntry(hMC_Prefit[j][i],Form("%s",topo_multipi_types_reduced[i].c_str()),"f");
        }
        legendTopo->SetBorderSize(0);
        //if (TKI==2) 
        legendTopo->SetFillStyle(0);
        //if (j==0&&TKI==1) 
        legendTopo->Draw();
    ...
    hMCTruth->GetXaxis()->SetTitle("True #it{#deltap_{TT}} (MeV/c)");
    hMCTruth->GetYaxis()->SetTitle("Selected events");
    hMCTruth->SetMarkerSize(1.);
    hMCTruth->SetLineWidth(3);
    hMCTruth->SetMarkerStyle(kFullCircle);
    hMCTruth->Draw();
    c3->SaveAs("dptt_truth.pdf");
...
void makePlots_xsec(){
    TFile* f = new TFile("/hepstore/kmtsui/T2K/work/xsLLhFitter_super/data_fit/all/xsec_dptt_prod6T_geomfixed_savetoys.root");
    const int nToys = 10;
    TH1D* hist_toy[nToys];
    TCanvas* c1 = new TCanvas();
    auto legendTopo = new TLegend(0.7,0.45,0.9,0.9);
    for (int i=0;i<nToys;i++) {
        hist_toy[i] = (TH1D*)f->Get(Form("sel_signal_toy%i",i));
        hist_toy[i]->SetBinContent(3,hist_toy[i]->GetBinContent(3)+hist_toy[i]->GetBinContent(6));
        hist_toy[i]->GetXaxis()->SetRangeUser(0,5);
        hist_toy[i]->GetYaxis()->SetRangeUser(0,0.6e-42);
        hist_toy[i]->SetLineColor(i+1);
        hist_toy[i]->SetLineWidth(2);
        hist_toy[i]->Draw("hist same");
        legendTopo->AddEntry(hist_toy[i],Form("Toy%i",i),"l");
    }
    legendTopo->Draw();
    c1->SaveAs(Form("xsec_dptt_toys_%i.pdf",nToys));
}

void makePlots_Samples_CEWG(){
    TStyle *t2kstyle = new TStyle("T2K","T2K approved plots style");
    SetStyleVariables(t2kstyle);
    gROOT->SetStyle("T2K");
