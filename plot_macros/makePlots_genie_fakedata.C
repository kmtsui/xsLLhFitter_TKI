...
void makePlots(){
  ...
  //TFile* fIn = new TFile("xsec_neutMC_genieData.root");
  TFile* fIn = new TFile("xsec_test_bkg.root");
  ...
  const int nBins = 5;
  const double Bins[nBins+1] = {-700,-300,-100,100,300,700};
  ...
  TFile* fIn_neut = new TFile("xsec_neut.root");
  TH1D* sel_best_fit_neut = (TH1D*)fIn_neut -> Get("sel_best_fit");
  TFile* fIn_genie = new TFile("xsec_genie.root");
